...
import storeCuration, { catalogModeSchema } from "models/store_curation";
...
async function postHandler(req: NextApiRequest, res: NextApiResponse) {
  const parsed = catalogModeSchema.safeParse(req.body);
  if (!parsed.success)
  ...
  return res
    .status(200)
    .json(await storeCuration.setCatalogMode(foundStore.id, parsed.data));
}
