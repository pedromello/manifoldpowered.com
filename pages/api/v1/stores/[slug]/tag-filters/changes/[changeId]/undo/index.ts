...
import storeCuration, { expectedRevisionSchema } from "models/store_curation";
...
async function postHandler(req: NextApiRequest, res: NextApiResponse) {
  ...
  return res
    .status(200)
    .json(
      await storeCuration.undoTagRuleChange(
        foundStore.id,
        req.query.changeId as string,
        parsed.data.expected_draft_revision,
      ),
    );
}
