...
async function postHandler(req: NextApiRequest, res: NextApiResponse) {
  const parsed = bulkCurationPreviewSchema.safeParse(req.body);
  if (!parsed.success)
  ...
  return res
    .status(200)
    .json(await storeCuration.previewBulkCuration(foundStore.id, parsed.data));
}
