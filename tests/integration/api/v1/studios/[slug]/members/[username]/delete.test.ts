...
describe("DELETE /api/v1/studios/[slug]/members/[username]", () => {
  describe("Owner", () => {
    test("Should remove the member and return 200", async () => {
      ...
      expect(responseBody).toEqual({ message: "Member removed from studio" });

      const memberInDatabase = await prisma.studioMember.findUnique({
        where: {
          studio_id_user_id: {
  ...
  describe("Removed member", () => {
    test("Should no longer be able to update the studio", async () => {
      const owner = await orchestrator.createUser();
      await orchestrator.activateUser(owner.id);
      const ownerSession = await orchestrator.createSession(owner.id);
      const createdStudio = await orchestrator.createStudio(owner.id);
