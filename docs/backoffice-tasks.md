...
- [x] **5. Games admin backend** — `game.findAllPaginatedAdmin({page, limit, status?, studio_id?, q})`, `game.setStatus(id, status)` (generalizes `makePublic`); `GET /api/v1/backoffice/games`, `PATCH /api/v1/backoffice/games/[slug]/status` (`reason` required for PRIVATE/INACTIVE), `PATCH .../bulk-status`; audit log writes; integration tests. _Depends on 1, 3._ ([#109](https://github.com/pedromello/manifoldpowered.com/pull/109))
- [x] **6. Users admin backend** — `user.findAllPaginated({page, limit, q})`; `GET /api/v1/backoffice/users`, `GET .../users/[id]`, `PATCH .../users/[id]/disable`, `PATCH .../users/[id]/enable`; self-protection guard (can't disable/de-admin self); audit log writes (with `previous_features` metadata); tests. _Depends on 1, 3, 4._ ([#110](https://github.com/pedromello/manifoldpowered.com/pull/110))
- [x] **7. Studios + Stores admin backend** — `studio.findAllPaginated` / `store.findAllPaginated`; `GET /api/v1/backoffice/studios`, `GET .../studios/[slug]` (incl. studio's games), `GET /api/v1/backoffice/stores`, `GET .../stores/[slug]`; tests. _Depends on 1._ ([#111](https://github.com/pedromello/manifoldpowered.com/pull/111))
- [x] **8. Dashboard backend** — aggregation function (`models/dashboard.ts` or extend `models/status.ts`): pending-games count + oldest N pending, signups this week vs last week, totals (users/studios/stores/games by status); `GET /api/v1/backoffice/dashboard`; tests. _Depends on 1._ ([#112](https://github.com/pedromello/manifoldpowered.com/pull/112))
