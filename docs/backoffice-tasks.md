...
- [x] **2. Admin bootstrap script** — `scripts/create-admin.ts` + `npm run admin:grant -- --email=<email>` (idempotent: find-or-create, activate if needed, grant `ADMIN_ONLY_FEATURES`). _Depends on 1._ ([#106](https://github.com/pedromello/manifoldpowered.com/pull/106))
...
- [x] **4. User disable/enable via features** — no `User.status` column; `authorization.ANONYMOUS_USER_FEATURES` / `DISABLED_USER_FEATURES` (anonymous set minus session/account-bootstrap features); `user.disable`/`user.enable`; `AdminActionLog.metadata` stores a `{previous_features}` snapshot for restore. _Depends on 1._ ([#108](https://github.com/pedromello/manifoldpowered.com/pull/108))
...
- [ ] **6. Users admin backend** — `user.findAllPaginated({page, limit, q})`; `GET /api/v1/backoffice/users`, `GET .../users/[id]`, `PATCH .../users/[id]/disable`, `PATCH .../users/[id]/enable`; self-protection guard (can't disable/de-admin self); audit log writes (with `previous_features` metadata); tests. _Depends on 1, 3, 4._
- [ ] **7. Studios + Stores admin backend** — `studio.findAllPaginated` / `store.findAllPaginated`; `GET /api/v1/backoffice/studios`, `GET .../studios/[slug]` (incl. studio's games), `GET /api/v1/backoffice/stores`, `GET .../stores/[slug]`; tests. _Depends on 1._
- [ ] **8. Dashboard backend** — aggregation function (`models/dashboard.ts` or extend `models/status.ts`): pending-games count + oldest N pending, signups this week vs last week, totals (users/studios/stores/games by status); `GET /api/v1/backoffice/dashboard`; tests. _Depends on 1._
- [ ] **9. Backoffice UI shell** — `BackofficeLayout`, nav, client-side auth-gate redirect. Tailwind + SWR + `lucide-react`, matching `components/store/UserMenu.tsx` conventions. _Depends on 1._
- [ ] **10. Games review queue UI** — `pages/backoffice/games/index.tsx`: status filter, search, multi-select approve, reject-with-reason modal. _Depends on 5, 9._
- [ ] **11. Users UI** — `pages/backoffice/users/index.tsx` + `[id].tsx`: search/list, detail, disable/enable with confirm dialog. _Depends on 6, 9._
- [ ] **12. Studios + Stores UI** — `pages/backoffice/studios/index.tsx` + `[slug].tsx`, `pages/backoffice/stores/index.tsx` + `[slug].tsx`: search/list, read-only detail. _Depends on 7, 9._
